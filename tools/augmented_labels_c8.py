...
for mf in ['train']:
  ...
  for img in img_data:
    ...
    src_img_path=os.path.join(src_img_root,mode+'_{}'.format(conf),fname)
  ...
  if not os.path.exists(res_path):
    ...
    for img in img_data:
      fname=img['file_name']
      src_img_path=os.path.join(src_img_root,'{}_c8'.format(mode),fname)
      if not os.path.exists(src_img_path):
        continue
  ...
  for idx,ann in enumerate(ann_data):
    ...
    # src_image_path=os.path.join(src_img_root,'train_c8',fname+'.png')
