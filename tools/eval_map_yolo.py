...
def parse_gt(fpath,binary=False,target_cls=None):
    ...
    for line in lines:
        ...
        # x0,y0,x1,y1=np.array(splits[1:]).astype(np.float32)*1025
        cat=int(splits[0])
        if target_cls is not None and (cat not in target_cls):
            continue
        if binary:
            cat=0
...
def parse_pred(fpath,binary=False,target_cls=None):
    lines=open(fpath).readlines()
    dst_list=[]
    # img_path=os.path.join(fpath.replace('yolo_preds','images/val_c6').replace('.txt','.jpeg'))
    # img=Image.open(img_path)
    for line in lines:
        line=line.strip()
        splits=line.split()
        x0,y0,x1,y1=np.array(splits[1:]).astype(np.float32)
        cat=int(splits[0])
        if target_cls is not None and (cat not in target_cls):
            continue
        if binary:
            cat=0
        dst_item=np.array([x0,y0,x1,y1,cat]).astype(np.int32)
        dst_list.append(dst_item)
    # drawn=visualize_box(img,dst_list)
    # drawn.save('pred.jpg')
    # exit()
...
mode='val'
pred_root='/home/twkim/project/yolov5_qlab07/sampled1K/{}/yolo_preds_{}'.format(mode,mode)
...
target_cls=[0,1,2]
for ff in flist:
    pred_path=os.path.join(pred_root,ff)
    gt_path=os.path.join(label_root,ff)
    if (not os.path.exists(pred_path)) or (not os.path.exists(gt_path)):
        continue
    # ground_truths.append(parse_gt(gt_path,binary=True))
